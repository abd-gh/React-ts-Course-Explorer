...
import { addDoc, collection, doc, getDocs, orderBy, onSnapshot, updateDoc, Timestamp } from "firebase/firestore";
import './rate.css';
...
const Rate = ({id,Name,Rate_Avg,Last_Review_Name,Last_Review_date,Points,count,updateRate}:course) => {
    ...
    const addRating = async (index:number, id:string, points:number, count:number):Promise<void> => {
        console.log("hello")
    ...
    const update = async (id:string, rate:number, points:number, count:number):Promise<void> => {
                ...
                {[...Array(5)].map((star, index) => {
                    ...
                    return (
                        <button
                            type="button"
                            key={index}
                            className={index <= (hover ||  Math.round(Rate_Avg)) ? "on" : "off"}
                             onClick={() =>{if(window.confirm('Do you really want to evaluate a '+Name+' course with '+index+' stars?')){addRating(index, id , Points, count)};} }
